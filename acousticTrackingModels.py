...
class IcoTempCNN(nn.Module):
	def __init__(self, r, C, Cin=1, smooth_vertices=True):
		super().__init__()
		self.r = r
		H, W = 2**r, 2**(r+1)
		deep = 5 + (r-1)*2
		self.process_vertices = nn.ModuleList([icoCNN.SmoothVertices(i+1) if smooth_vertices else icoCNN.CleanVertices(i+1)
											   for i in range(r)])
		self.ico_cnn = nn.ModuleList([icoCNN.ConvIco(r, Cin, C, 1, 6, smooth_vertices=smooth_vertices)] +
									 [icoCNN.ConvIco(max(r-i//2, 1), C, C, 6, 6, smooth_vertices=smooth_vertices)
									  for i in range(1, deep)])
		self.temp_cnn = nn.ModuleList([at_modules.CausConv1d(C, C, 5, 1) for i in range(0, deep-1)] +
									  [at_modules.CausConv1d(C, 1, 5, 1)])
		self.layer_norm = nn.ModuleList([icoCNN.LNormIco(C, 6) for i in range(deep)])
		self.poolings = nn.ModuleList([icoCNN.PoolIco(r-i, 6) for i in range(r)])

		self.clean_vertices = icoCNN.CleanVertices(1)
		ico_grid = torch.from_numpy(icoCNN.icosahedral_grid_coordinates(1))
		ico_grid = rearrange(ico_grid, 'charts H W coors -> coors charts H W', charts=5, coors=3)
	...
	def apply_cnn(self, x):
		...
		for idx, (ico_conv, temp_conv, l_norm) in enumerate(zip(self.ico_cnn, self.temp_cnn, self.layer_norm)):
			...
			x = rearrange(x, 'B T C R charts H W -> (B R charts H W) C T', B=B, R=6, charts=5)
			x = temp_conv(x)
			x = rearrange(x, '(B R charts H W) C T -> B T C R charts H W', R=6, charts=5, B=B, H=H, W=W)
			...
			if idx % 2 == 1 and r > 1:
				...
				H //= 2
				W //= 2
		x = reduce(x, 'B T C R charts H W -> B T charts H W', 'max', R=6, charts=5)
		return x
